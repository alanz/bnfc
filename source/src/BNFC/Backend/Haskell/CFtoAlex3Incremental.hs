...
restOfAlex shareMod shareStrings byteStrings cf = [
  ...
  "{",
  "",
  -- "tokens :: FT.Measured v IntToken => String -> FT.FingerTree v IntToken",
  -- "tokens str = stateToTree $ FT.measure $ makeTree str",
  ...
  "unescapeInitTail = unesc . tail . Data.Foldable.toList where",
  "  unesc s = case s of",
  ...
  "            | Tokens { currentSeq  :: !(FT.FingerTree v IntToken)",
  "                      , lastToken  :: !(Suffix v)",
  "                      , outState   :: !State}",
  ...
  "type Size     = Sum Int",
  "--Wrapper",
  "type LexTree v = FT.FingerTree (Table State (Tokens v),Size) Char",
  "data IntToken  = Token { lexeme   :: !(S.Seq Char)",
  "                       , token_id :: Accepts",
  "                       , action_fn :: Posn -> S.Seq Char -> Token}",
  "-- Accepts type models the alexScanUser function in the alex wrapper, for the AlexLastAcc case",
  "--Wrapper template",
  -- "type Accepts   = AlexAcc (Posn -> S.Seq Char -> Token)",
  "type Accepts   = AlexAcc ()",
  ...
  "access a x = a ! x",
  "-- Generic template",
  "makeTree :: FT.Measured v IntToken => String -> LexTree v",
  "makeTree str = FT.fromList str",
  "",
  "-- Generic template",
  "instance (FT.Measured v IntToken) => Monoid (Table State (Tokens v)) where",
  ...
  "instance (FT.Measured v IntToken) => FT.Measured (Table State (Tokens v),Size) Char where",
  "  measure c =",
  ...
  "            AlexAccNone -> Tokens FT.empty (Str cSeq) os",
  "            acc -> Tokens FT.empty (One (createToken cSeq acc)) os",
  "    in (tabulate stateRange $ baseCase, Sum 1)",
  "",
  "intToks :: FT.Measured v IntToken => FT.FingerTree v IntToken -> Suffix v -> FT.FingerTree v IntToken",
  "intToks seq (Str str) = error $ \"Unacceptable token: \" ++ Data.Foldable.toList str",
  "intToks seq (One token) = seq FT.|> token",
  "intToks seq (Multi (Tokens seq' suff' _)) = intToks (seq <> seq') suff'",
  "",
  "stateToTree :: FT.Measured v IntToken => (Table State (Tokens v),Size) -> FT.FingerTree v IntToken",
  "stateToTree (tab,size) = case access tab startState of",
  "    InvalidTokens s          -> error $ \"Unacceptable token: \" ++ Data.Foldable.toList s",
  "    NoTokens                 -> FT.empty",
  ...
  "createToken :: S.Seq Char -> Accepts -> IntToken",
  "createToken lex acc@(AlexAcc k) = Token lex acc (alex_actions ! k)",
  "createToken lex acc             = Token lex acc (error \"createToken\")",
  "",
  "createTokens :: FT.Measured v IntToken => FT.FingerTree v IntToken -> Suffix v -> State -> Tokens v",
  "createTokens seq suf state = if FT.null seq",
  "                             then NoTokens",
  ...
  "combineTokens :: FT.Measured v IntToken => Transition v -> Transition v -> Transition v",
  ...
  "combineWithRHS :: FT.Measured v IntToken => Tokens v -> Transition v -> Tokens v",
  ...
  "      in appendTokens seq1 toks2'",
  "    One tok -> appendTokens (seq1 FT.|> tok) (trans2 startState)",
  ...
  "mergeTokens :: FT.Measured v IntToken => Suffix v -> Tokens v -> Transition v -> Tokens v",
  "mergeTokens suff1 toks2 trans2 = case FT.viewl (currentSeq toks2) of",
  "  token2 FT.:< seq2' -> let newToken = mergeToken suff1 token2",
  "                        in toks2 {currentSeq = newToken FT.<| seq2'}",
  "  FT.EmptyL -> case alex_accept ! out_state of",
  ...
  "mergeSuff :: FT.Measured v IntToken => Suffix v -> Suffix v -> Transition v -> Suffix v",
  ...
  "  in if isValid toks2",
  "     then Multi $ toks2 {currentSeq = token1 FT.<| currentSeq toks2}",
  "     else Multi $ createTokens (FT.singleton token1) (Str s) (-1)",
  ...
  "appendTokens :: FT.Measured v IntToken => FT.FingerTree v IntToken -> Tokens v -> Tokens v",
  ...
  "",
  "-- Genereic template",
  "concatLexemes :: FT.FingerTree v IntToken -> S.Seq Char",
  ...
  "automata (I# s) c = let base   = alexIndexInt32OffAddr alex_base s",
  "                        ord_c = case fromIntegral c of (I# ord_c') -> ord_c'",
  "                        offset = (base +# ord_c)",
