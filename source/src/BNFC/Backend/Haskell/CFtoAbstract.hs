...
cf2Abstract :: Bool -- ^ Generate incremental parser
            ...
            -> CF       -- ^ Grammar
            -> String
cf2Abstract incremental byteStrings ghcExtensions functor name cf = unlines $
  ...
  (map (render . \c -> prSpecialData incremental byteStrings (isPositionCat cf c) derivingClasses c) (specialCats cf)
...
prSpecialData :: Bool     -- ^ If True, use incremental option
              -> Bool     -- ^ If True, use ByteString instead of String
              -> Bool     -- ^ If True, store the token position
              -> [String] -- ^ Derived classes
              -> Cat      -- ^ Category
              -> Doc
prSpecialData incremental byteStrings position classes cat =
